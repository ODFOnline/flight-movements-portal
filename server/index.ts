...
// ---------- TEMP: one-time admin seed/reset via URL ----------
...
app.post('/api/admin/upload-roster', requireAuth, requireRole('ADMIN'), upload.single('roster'), async (req: any, res: any) => {
  try {
    ...
    // Try to detect the header that contains the full name
    const normalize = (s: string) => String(s || '').trim();
    let imported = 0;

    for (const r of rows) {
      // Accept several common headers: Full Name, Name, Member, Traveler, Passenger
      const fullName =
      ...
      // Create a one-time enrollment code (optional future self-register)
...
app.get('/api/movements', requireAuth, async (req: any, res: any) => {
  ...
  if (isAdmin && q) {
    members = await prisma.member.findMany({
      ...
      include: { flights: { orderBy: [{ departDT: 'asc' }, { segmentIdx: 'asc' }] }, itineraries: { orderBy: { uploadedAt: 'desc' } } },
    });
  } else if (isAdmin && !q) {
    members = await prisma.member.findMany({
      take: 100,
...
// ---------- ensure DB & Prisma client exist BEFORE using Prisma ----------
function ensureDb() {
  try {
